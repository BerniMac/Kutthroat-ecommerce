...
if ($cart_id !== '') {
    ...
    $cartQ = $DBConnect->query("SELECT * FROM cart WHERE id = '{$cart_id}' LIMIT 1");
    if ($cartQ && $cartQ->num_rows > 0) {
        ...
        $DBConnect->query("UPDATE cart SET items = '{$items_json}', expire_date = '{$cart_expire}' WHERE id = '{$cart_id}'");
...
if ($cart_id === '') {
    ...
    $DBConnect->query("INSERT INTO cart (items, expire_date) VALUES ('{$items_json}', '{$cart_expire}')");
...
if ($cart_id !== '') {
    ...
    if ($cartQ && $cartQ->num_rows > 0) {
        ...
        $items_json  = $DBConnect->real_escape_string(json_encode($new_items));
        $cart_expire = date("Y-m-d H:i:s", strtotime("+30 days"));
        $DBConnect->query("UPDATE cart SET items = '{$items_json}', expire_date = '{$cart_expire}' WHERE id = '{$cart_id}'");
...
if ($cart_id === '') {
    $items_json  = $DBConnect->real_escape_string(json_encode($item));
    $cart_expire = date("Y-m-d H:i:s", strtotime("+30 days"));
    $DBConnect->query("INSERT INTO cart (items, expire_date) VALUES ('{$items_json}', '{$cart_expire}')");
    $cart_id = $DBConnect->insert_id;
    setcookie(CART_COOKIE, $cart_id, CART_COOKIE_EXPIRE, '/', $domain, false);
