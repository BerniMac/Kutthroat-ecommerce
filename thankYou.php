...
try {
  ...
  $itemQ = $DBConnect->query("SELECT * FROM cart WHERE id = '{$cart_id}'");
  $iresults = mysqli_fetch_assoc($itemQ);
  $items = json_decode($iresults['items'], true);
  foreach ($items as $item) {
    $newSizes = array();
    $item_id = $item['id'];
    $productQ = $DBConnect->query("SELECT * FROM products WHERE id = '{$item_id}'");
    ...
    $DBConnect->query("UPDATE products SET sizes = '{$sizeString}' WHERE id = '{$item_id}'");
  }

  //update cart
  $DBConnect->query("UPDATE cart SET paid = 1 WHErE id = '{$cart_id}'");
  $DBConnect->query("INSERT INTO transactions
